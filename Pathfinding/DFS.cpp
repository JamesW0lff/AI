...
bool DFS::Search(GridBasedGraph& graph, int fromColumn, int fromRow, int toColumn, int toRow, function<bool(int, int)> isBlocked)
{
		...
		else
		{
			...
			for (int i = 0; i < neighbors; ++i) {
				...
				if (!(nextNode->neighbours[i]->opened || nextNode->neighbours[i]->closed))
				{
					...
					mOpenList.push_front(nextNode->neighbours[i]);
				}
